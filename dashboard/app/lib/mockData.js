...
import { addLog, addSession } from './db';
...
export async function seedMockData() {
  ...
  for (let dayOffset = DAYS - 1; dayOffset >= 0; dayOffset--) {
    ...
    for (const profile of profiles) {
      for (const hour of profile.hours) {
        ...
        const tsEnd = new Date(ts.getTime() + duration * 1000);

        // Activity log entry (for charts/analytics)
        logEntries.push({
          site, duration,
        ...
        // Session entry (for Session History tab)
        sessionEntries.push({
          type: 'background',
          domain: site,
          title: '',
          ...
          category: profile.cat,
          sessionGroup: ts.toISOString().split('T')[0],
          source: 'demo',
        });

        // 40% chance of a second short visit in the same hour
        ...
        if (secondVisit >= 6) {
          ...
          const ts2End = new Date(ts2.getTime() + dur2 * 1000);

          logEntries.push({
            site: site2, duration: dur2,
          ...
          sessionEntries.push({
            ...
            domain: site2,
            title: '',
            startTime: ts2.toISOString(),
            endTime: ts2End.toISOString(),
            durationMs: dur2 * 1000,
            category: profile.cat,
            sessionGroup: ts2.toISOString().split('T')[0],
            source: 'demo',
          });
        }
      }
  ...
  // Write all sessions (limited to most recent 60 for performance)
  const recentSessions = sessionEntries.slice(-60);
