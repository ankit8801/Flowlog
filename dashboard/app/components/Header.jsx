...
export default function Header() {
  const {
    ...
    activeTab, setActiveTab, cognitiveState, cfs
  } = useFocusStore();
  ...
  return (
    <header className="header">
      ...
      <div className="header-right">
        ...
        <div className={`cognitive-state-badge state-${cognitiveState.toLowerCase()}`} title={`CFS: ${cfs}/100`}>
          {cognitiveState === 'FOCUSED' && '⚡ Deeply Focused'}
          {cognitiveState === 'DRIFTING' && '🌀 Drifting'}
          {cognitiveState === 'DISTRACTED' && '⚠️ Distracted'}
          {cognitiveState === 'DEEPLY_DISTRACTED' && '🔴 Deeply Distracted'}
        </div>
