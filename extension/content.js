...
(function () {
  ...
  function isExtensionContextValid() {
    try {
      ...
      document.getElementById('ft-intervention-root')?.remove();
      document.getElementById('ft-toast-root')?.remove();
      document.getElementById('ft-block-root')?.remove();
      return false;
    }
  }

  // Debugging Storage Listener (Fix 4)
  if (isExtensionContextValid()) {
    chrome.storage.onChanged.addListener((changes, area) => {
      if (!isExtensionContextValid()) return;
      if (area === 'local' && changes.productiveDomains) {
        console.log('[FocusTracker] productiveDomains updated:', changes.productiveDomains.newValue);
      }
    });
  }
  ...
  /**
   * playAlertSound — generates a soft, pleasant notification tone
   * using the Web Audio API (no external MP3 needed).
   */
  async function playAlertSound() {
    try {
      // Avoid autoplay warnings by never touching WebAudio before user gesture.
      if (!navigator.userActivation?.hasBeenActive) return;

      const AudioCtx = window.AudioContext || window.webkitAudioContext;
      if (!AudioCtx) return;

      // Reuse one context and only play after a successful resume.
      const ctx = window.__ftAlertAudioCtx || new AudioCtx();
      window.__ftAlertAudioCtx = ctx;
      ...
      osc.frequency.exponentialRampToValueAtTime(659.25, ctx.currentTime + 0.1); // E5 note
      ...
      console.log('[FocusTracker] Audio context blocked or unavailable.');
    }
  }
  /**
  ...
  async function shouldShowIntervention(domain, pathname = window.location.pathname) {
    if (!isExtensionContextValid()) return false;
    
    const stored = await chrome.storage.local.get(['productiveDomains', 'blockedSites']);
    const productiveDomains = stored.productiveDomains ?? [];
    const blockedSites      = stored.blockedSites ?? [];
    
    console.log('[FocusTracker] checking domain:', domain);
    console.log('[FocusTracker] productiveDomains:', productiveDomains);
    console.log('[FocusTracker] blockedSites:', blockedSites);
    
    const cleanDomain = (domain || '').toLowerCase().replace(/^www\./, '');
    const cleanPath = (pathname || '').replace(/\/$/, '').toLowerCase();
    const currentPath = (cleanDomain + cleanPath).toLowerCase();
    ...
    function matchesEntry(entry) {
      ...
      return cleanDomain === normalized || cleanDomain.endsWith('.' + normalized);
    }

    if (blockedSites.some(matchesEntry)) {
      console.log('[FocusTracker] domain/path matched blockedSites, forcing intervention popup');
      return true;
    }

    if (productiveDomains.some(matchesEntry)) {
      console.log('[FocusTracker] domain is productive, skipping generic intervention popup');
      return false;
    }

    return DISTRACTING.has(cleanDomain);
  }

  /**
   * maybeIntervene — gated entry point for the intervention system.
   */
  async function maybeIntervene(currentHostname) {
    if (!isExtensionContextValid()) return;
    ...
    const show = await shouldShowIntervention(currentHostname);
    if (!show) return;
  ...
  function runFocusCheck() {
    ...
    chrome.storage.local.get(
      ['focusEnabled', 'blockedSites', 'productiveDomains', 'allowContinue', 'sensitivity', 'sessionStart', 'sessionDuration'],
      function (data) {
        ...
        function isMatch(tabDomain, storedDomain) {
          if (!tabDomain || !storedDomain) return false;
          const tab = tabDomain.toLowerCase().replace(/^www\./, '');
          const stored = storedDomain.toLowerCase().replace(/^www\./, '');
          return tab === stored || tab.endsWith('.' + stored);
        }

        const blocked = Array.isArray(data.blockedSites) ? data.blockedSites : [];
        const isBlocked = blocked.some(entry => {
          if (entry.includes('/')) return currentPath === entry.toLowerCase() || currentPath.startsWith(entry.toLowerCase() + '/');
          return isMatch(hostname, entry);
        });
        ...
        if (isBlocked) {
          ...
          if (!document.getElementById(OVERLAY_ID)) {
            ...
            // Also show popup for blocked lists per requirement
            showInterventionPopup(hostname);
          }
          return;
        }

        const productive = Array.isArray(data.productiveDomains) ? data.productiveDomains : [];
        const isProductive = productive.some(entry => {
          if (entry.includes('/')) return currentPath === entry.toLowerCase() || currentPath.startsWith(entry.toLowerCase() + '/');
          return isMatch(hostname, entry);
        });
        ...
        if (isProductive) {
          ...
          removeBlockOverlay();
          removeToast();
          return; // productive sites bypass warnings and popups
        }
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    switch (msg.type) {
      ...
      case 'SHOW_INTERVENTION':
        shouldShowIntervention(msg.site || hostname).then((show) => {
          if (show) showInterventionPopup(msg.site || hostname);
        });
        break;
    }
  });
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes.focusEnabled || changes.blockedSites || changes.productiveDomains) setTimeout(runFocusCheck, 100);
  });
